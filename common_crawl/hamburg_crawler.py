...
class HamburgCrawler(BaseCommonCrawler):
    def __init__(self, output_dir="data/hamburg_raw", headless=True, limit=None, overwrite=False, skip_threshold=50, stealth=False):
        super().__init__(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Hamburg Landesrecht Crawler")
    ...
    parser.add_argument("--output", type=str, default="data/hamburg_raw", help="Output directory")

    args = parser.parse_args()

    crawler = HamburgCrawler(output_dir=args.output, headless=not args.no_headless, limit=args.limit, overwrite=args.overwrite, skip_threshold=args.skip_threshold, stealth=args.stealth)
    crawler.crawl()
